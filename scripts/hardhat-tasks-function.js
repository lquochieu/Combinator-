...
const { encrypt, decrypt } = require('./utils/cryto');

async function changeWbnbAddress(oldNetworkName, newNetworkName) {
    ...
    fs.writeFileSync(
        ...
        tokenUtilsContract.replaceAll(
            hardhatSettings.wbnbAddress[oldNetworkName],
            hardhatSettings.wbnbAddress[newNetworkName],
        ),
    );
}
...
async function changeNetworkNameForAddresses(oldNetworkName, newNetworkName) {
    ...
    changeWbnbAddress(oldNetworkName, newNetworkName);
