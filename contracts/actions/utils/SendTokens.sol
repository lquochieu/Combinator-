...
contract SendTokens is ActionBase {
    ...
    function executeAction(
        ...
        for (uint256 i = 0; i < arrayLength; ) {
            inputData.tokens[i] = _parseParamAddr(
                inputData.tokens[i],
                _paramMapping[i],
                _subData,
                _returnValues
            ...
            inputData.receivers[i] = _parseParamAddr(
                ...
                _paramMapping[arrayLength + i],
                _subData,
                _returnValues
            );
            inputData.amounts[i] = _parseParamUint(
                inputData.amounts[i],
                _paramMapping[2 * arrayLength + i],
                _subData,
                _returnValues
            );
    ...
    function _sendTokens(
        ...
        for (uint256 i = 0; i < arrayLength; ) {
            _inputData.tokens[i].withdrawTokens(
                _inputData.receivers[i],
    ...
    function parseInputs(
        bytes memory _callData
    ) public pure returns (Params memory params, uint256 arrayLength) {
