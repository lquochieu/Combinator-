...
const claimStkTrava = async (proxy, assets, amount, to) => {
  const aaveClaimStkTravaAction = new dfs.actions.aave.TravaClaimStkTravaAction(
    assets,
    amount,
    to
  );
...
const reflexerOpen = async (proxy, adapterAddr) => {
  const openMySafe = new dfs.actions.reflexer.ReflexerOpenSafeAction(
    adapterAddr
  );
...
const reflexerWithdraw = async (proxy, safeId, amount, adapterAddr, to) => {
  const reflexerWithdrawAction =
    new dfs.actions.reflexer.ReflexerWithdrawAction(
      safeId,
      amount,
      adapterAddr,
      to
...
const reflexerGenerate = async (proxy, safeId, amount, to) => {
  const reflexerGenerateAction =
    new dfs.actions.reflexer.ReflexerGenerateAction(safeId, amount, to);
...
const balancerWithdraw = async (
  ...
  const balancerWithdrawAction =
    new dfs.actions.balancer.BalancerV2WithdrawAction(
      poolId,
      from,
      to,
      lpTokenAmount,
...
const openMcd = async (proxy, joinAddr, mcdManager = MCD_MANAGER_ADDR) => {
  const openMyVault = new dfs.actions.maker.MakerOpenVaultAction(
    joinAddr,
    mcdManager
  );
...
const openVault = async (proxy, collType, collAmount, daiAmount) => {
  ...
  const amountColl = hre.ethers.utils.parseUnits(
    collAmount,
    tokenData.decimals
  );
...
const mcdFLRepayComposite = async (
  ...
  const repayCompositeAction =
    new dfs.actions.maker.MakerFLRepayCompositeAction(
      vaultId,
      joinAddr,
      gasUsed,
      exchangeParams
...
const mcdFLBoostComposite = async (
  ...
  const boostCompositeAction =
    new dfs.actions.maker.MakerFLBoostCompositeAction(
      vaultId,
      joinAddr,
      gasUsed,
      exchangeParams
...
const uniSupply = async (
  ...
  const uniSupplyAction = new dfs.actions.uniswap.UniswapSupplyAction(
    ...uniObj
  );
...
const uniWithdraw = async (
  ...
  const uniWithdrawAction = new dfs.actions.uniswap.UniswapWithdrawAction(
    ...uniObj
  );
...
const liquitySPWithdraw = async (proxy, LUSDAmount, to, wethTo, lqtyTo) => {
  const liquitySPWithdrawAction =
    new dfs.actions.liquity.LiquitySPWithdrawAction(
      LUSDAmount,
      to,
      wethTo,
      lqtyTo
...
const liquityEthGainToTrove = async (proxy, lqtyTo) => {
  ...
  const { ethGain } = await liquityView["getDepositorInfo(address)"](
    proxy.address
  );
  const { upperHint, lowerHint } = await getHints(
    proxy.address,
  ...
  const liquityEthGainToTroveAction =
    new dfs.actions.liquity.LiquityEthGainToTroveAction(
      lqtyTo,
      upperHint,
      lowerHint
    );
...
const curveGaugeWithdraw = async (
  ...
  const curveGaugeWithdrawAction =
    new dfs.actions.curve.CurveGaugeWithdrawAction(
      gaugeAddr,
      lpToken,
      receiver,
      amount
...
const curveStethPoolDeposit = async (
  ...
  const curveStethPoolDepositAction =
    new dfs.actions.curve.CurveStethPoolDepositAction(
      from,
      to,
      amounts,
      minMintAmount
...
const curveStethPoolWithdraw = async (
  ...
  const curveStethPoolWithdrawAction =
    new dfs.actions.curve.CurveStethPoolWithdrawAction(
      from,
      to,
      amounts,
      minBurnAmount,
...
const qiDaoOpen = async (proxy, vaultId) => {
  dfs.configure({
    chainId: 10,
  });
  const qidaoOpenAction = new dfs.actions.qidao.QiDaoOpenVaultAction(vaultId);
...
const qiDaoWithdraw = async (proxy, vaultId, userVaultid, amount, to) => {
  dfs.configure({
    chainId: 10,
  });
...
const qiDaoGenerate = async (proxy, vaultId, userVaultId, amount, to) => {
  dfs.configure({
    chainId: 10,
  });
...
const qiDaoPayback = async (proxy, vaultId, userVaultId, amount, from) => {
  dfs.configure({
    chainId: 10,
  });
...
const morphoTravaV3Supply = async (
  ...
  const morphoTravaSupplyAction =
    new dfs.actions.morpho.MorphoTravaV3SupplyAction(
      emodeId,
      tokenAddr,
      amount,
      from,
  ...
  const receipt = await proxy["execute(address,bytes)"](
    supplyAddr,
    functionData,
    { gasLimit: 3000000 }
  );
...
const morphoTravaV3Withdraw = async (
  ...
  const morphoTravaWithdrawAction =
    new dfs.actions.morpho.MorphoTravaV3WithdrawAction(
      emodeId,
      tokenAddr,
      amount,
      to,
...
const morphoTravaV3Payback = async (
  ...
  const morphoTravaPaybackAction =
    new dfs.actions.morpho.MorphoTravaV3PaybackAction(
      emodeId,
      tokenAddr,
      amount,
      from,
...
const morphoTravaV3Borrow = async (
  ...
  const morphoTravaBorrowAction =
    new dfs.actions.morpho.MorphoTravaV3BorrowAction(
      emodeId,
      tokenAddr,
      amount,
      to,
...
const aaveV3SwapBorrowRate = async (proxy, assetId, rateMode) => {
  const aaveSwapRateAddr = await getAddrFromRegistry(
    "TravaV3SwapBorrowRateMode"
  );

  const aaveSwapRateAction =
    new dfs.actions.aaveV3.TravaV3SwapBorrowRateModeAction(
      true,
      nullAddress,
      rateMode,
      assetId
...
const aaveV3ClaimRewards = async (proxy, assets, amount, to, rewardsAsset) => {
  const aaveClaimRewardsAction =
    new dfs.actions.aaveV3.TravaV3ClaimRewardsAction(
      assets.length,
      amount,
      to,
      rewardsAsset,
...
const aaveV3SwitchCollateral = async (
  ...
  const aaveSwitchCollateralAddr = await getAddrFromRegistry(
    "TravaV3CollateralSwitch"
  );
  const aaveSwithCollAction =
    new dfs.actions.aaveV3.TravaV3CollateralSwitchAction(
      true,
      market,
      arrayLength,
      tokens,
...
const morphoTravaV2Withdraw = async (proxy, tokenAddr, amount, to) => {
